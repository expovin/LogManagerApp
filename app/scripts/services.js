...
angular.module('QLog')
        ...
        .factory('$sessionStorage',['$window', function($window) {
            ...
            return {
                ...
                getCurrentNav : function (Server) {
                    var currentNav = $window.sessionStorage['currentNav'];
/*
                    if(currentNav === undefined){
                        var logs = this.get(Server);
                        var activeLayer = Object.keys(logs)[0];
                        var activeDir = Object.keys(logs[$scope.activeLayer])[0];
                        var activeFile = Object.keys(logs[$scope.activeLayer][$scope.activeDir])[0];

                        var currentFile['File'] = activeFile;
                        var currentDir['Dir'] = activeDir
                        var currentLayer['Layer'] = activeFile
                        var currentNav[Server] = [];
                        currentNav[Server].push(currentFile);
                        currentNav[Server].push(currentDir);
                        currentNav[Server].push(currentLayer);
                    }
                    */
                    console.log(currentNav[Server]);
                    return currentNav[Server];
